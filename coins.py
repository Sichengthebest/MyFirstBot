...
def yearly(update,context):
    ...
    if datetime.now() > hourlytime:
        ...
        update.message.reply_text("Slow it down, cmon!!! I'm not made of money dude, maybe you do hate 2021, but New Year did not pass yet!\n放慢速度，呆瓜！我不是用钱做的，也许您确实讨厌2021年，但新年还没有过去！")
...
def get_coins(update, context):
    user = update.effective_user
    check_user(user)
    update.message.reply_animation('https://i.pinimg.com/originals/6f/0b/b4/6f0bb4cbfbfd2bc10bd0ee85bdec6843.gif',caption=f"{show_user(user)}")
